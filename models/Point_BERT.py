...
class PointTransformerGenerator(nn.Module):
    ...
    def log_cloud(self, pts, mask_ratio=1):
        ...
        gt_logits = gt_logits.argmax(-1).long() # B G 
        ...
        masked_group_input_tokens = group_input_embeddings * (1 - w) + mask_token * w

        pos = self.pos_embed(center)
        logits = self._tokens_to_logits(masked_group_input_tokens.long(), pos)
        ...
        feature = self.dvae.dgcnn_2(sampled_max_tokens, center)
...
class MaskTransformer(nn.Module):
    ...
    def forward(self, neighborhood, center, return_all_tokens = False, only_cls_tokens = False, noaug = False):
        ...
        group_input_embeddings = self.reduce_dim(group_input_embeddings)
        # replace the point
        replaced_group_input_embeddings, overall_mask = self._random_replace(group_input_embeddings, bool_masked_pos.clone(), noaug = noaug)
        batch_size, seq_len, _ = replaced_group_input_embeddings.size()
        ...
        # mask the input tokens
        w = bool_masked_pos.unsqueeze(-1).type_as(mask_token)
        maksed_group_input_embeddings = replaced_group_input_embeddings * (1 - w) + mask_token * w
        # add pos embedding
        pos = self.pos_embed(center)
        # final input
        x = torch.cat((cls_tokens, maksed_group_input_embeddings), dim=1)
        pos = torch.cat((cls_pos, pos), dim=1)
        # transformer
        x = self.blocks(x, pos)
        x = self.norm(x)
