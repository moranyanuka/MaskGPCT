    ...
    if opti_config.type == 'AdamW':
        def add_weight_decay(model, weight_decay=1e-5, skip_list=()):
            ...
            for name, param in model.module.named_parameters():
                ...
                if len(param.shape) == 1 or name.endswith(".bias") or 'token' in name or 'embed' in name or name in skip_list:
                    # print(name)
...
def criterion_builder(config, device):
    matcher = build_matcher(config)
    weight_dict = {'loss_ce': 1, 'loss_center': config.center_loss_coef}
        # TODO this is a hack
    if config.model.transformer_config.aux_loss:
        aux_weight_dict = {}
        for i in range(config.model.transformer_config.decoder_depth - 1):
            aux_weight_dict.update({k + f'_{i}': v for k, v in weight_dict.items()})
        weight_dict.update(aux_weight_dict)
    losses = ['labels', 'centers', 'cardinality']
    criterion = SetCriterion(config.model.dvae_config.num_tokens, matcher=matcher, weight_dict=weight_dict,
